...
class EventMediaPausedVideoTest extends CaliperTestCase {
    function setUp() {
        ...
        $this->setTestObject((new MediaEvent())
            ...
            ->setObject((new VideoObject('https://example.edu/UQVK-dsU7-Y'))
                ->setName('Information and Welcome')
                ->setMediaType('video/ogg')
                ->setDuration('PT20M20S')
            )
            ->setTarget((new MediaLocation('https://example.edu/UQVK-dsU7-Y?t=321'))
                ->setCurrentTime('PT05M21S')
            )
            ->setEventTime(new \DateTime('2016-11-15T10:15:00.000Z'))
            ->setEdApp(new SoftwareApplication('https://example.edu/player'))
